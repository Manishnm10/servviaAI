...
export function AmbientBackground({ isVisible = true }: { isVisible?: boolean }) {
  ...
  useEffect(() => {
    ...
    const MOUSE_RADIUS = 300; // Larger radius — reacts sooner
    ...
    const resize = () => {
      ...
      particles = [];
      const count = Math.min(Math.floor(window.innerWidth / 10), 220); // ~1.5x more particles
      ...
      particles.forEach((p, i) => {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist2 < MAX_DISTANCE) {
            ...
            ctx.strokeStyle = `rgba(220, 38, 38, ${Math.min(opacity * 1.2, 1)})`; // Brighter lines
  ...
  return (
    <canvas 
      ref={canvasRef} 
      className={`fixed inset-0 w-full h-full z-[-1] pointer-events-none transition-opacity duration-[1800ms] ease-in ${isVisible ? 'opacity-100' : 'opacity-0'}`} 
