...
from legacy_agriculture.database.db_operations import create_record, get_record_by_field, update_record
from legacy_agriculture.database.models import (
    Conversation,
    FollowUpQuestion,
    Language,
    Messages,
...
from legacy_agriculture.language_service.utils import get_language_by_code
...
def send_request(
    ...
    try:
        ...
        response = session.send(
            request_prepped,
            stream=True,
            # verify=certifi.where(),  # Disabled due to SSL certificate error
            verify=False,  # FIXED: Bypass SSL verification
        )
